...
class NNLM(nn.Module):
    ...
    def forward(self, context, target):
        embeds = self.embedding(context).view(context.size(0), -1)
        proj = self.hidden_layer(self.projection(embeds))
...
model = NNLM(VOCAB_SIZE, EMBEDDING_DIM, CONTEXT_SIZE, HIDDEN_DIM, huffman_tree)
